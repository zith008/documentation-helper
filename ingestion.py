...
async def index_document_async(documents: List[Document], batch_size: int = 50):
    ...
    log_info(
        f"🗄️  VectorStore: Indexing: Preparing to add {len(documents)} documents to vector store",
        Colors.GREEN,
    )
    # create batches
    batches = [
    ...
    # Nested so it can access `batches` for logging
...
async def main():
    ...
    log_success(
        f"TavilyCrawl: Successfully crawled {len(all_docs)} pages from documentation site"
    )
